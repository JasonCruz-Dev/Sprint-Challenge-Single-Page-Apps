...
> With Server-Side routing, everytime a request is sent the entire page will need to be refreshed, which means unnecessary data is being requested. Eats up more computer cycles. The benefits of Client-Side routing is that less-data is processed, routing between views is generally faster, hence the end user will have smoother transitions and animations.
